...
# OI threshold slider
min_oi = st.sidebar.slider(
    "Minimum OI Threshold ($)",
    min_value=1000,
    max_value=500000,
    value=50000,
    step=10000,
    help="Only fetch and display markets with open interest >= this value"
)

# Fetch button
if st.sidebar.button("🔄 Fetch Market Data", key="fetch_button"):
    with st.spinner(f"Fetching markets (volume ≥ ${min_volume:,.0f}, OI ≥ ${min_oi:,.0f})..."):
        try:
            asyncio.run(fetch_data(min_volume, min_oi))
...
with col4:
    st.metric("Avg OI", f"${df['open_interest'].mean():,.0f}")
...
with col2:
    fig_scatter = px.scatter(
        ...
        labels={"open_interest": "Open Interest ($)", "volume": "Volume ($)"}
    )
